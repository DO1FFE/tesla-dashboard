...
5. On the configuration page (`/config`) you can set your APRS call sign, passcode and an optional comment to transmit position packets via an EU APRS-IS server. The outside temperature is also sent as APRS telemetry. The sequence number for these telemetry packets is stored in `data/aprs_seq_<vehicle_id>.txt` to avoid duplicates after a restart.
