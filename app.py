...
FLEET_TELEMETRIE_STREAM_WIEDERHERSTELLUNG_WIEDERHOLUNG_SECONDS = max(
    15.0,
    float(os.getenv("TESLA_FLEET_STREAM_RECOVERY_RETRY_SECONDS", "30")),
)
...
FLEET_TELEMETRIE_PROFILE_LIVE_POSITION_WIEDERAUFNAHME_SECONDS = max(
    0.5,
    float(
        os.getenv(
            "TESLA_FLEET_TELEMETRY_LIVE_POSITION_RESUME_SECONDS",
            "2",
        )
    ),
)
...
FLEET_TELEMETRIE_PROFILE_LIVE_NEUVERSAND_INTERVAL_SECONDS = max(
    3.0,
...
FLEET_TELEMETRIE_PROFILE_LIVE_BESTAETIGUNG_MAX_ALTER_SECONDS = max(
    2.0,
    float(os.getenv("TESLA_FLEET_TELEMETRY_LIVE_CONFIRM_MAX_AGE_SECONDS", "3")),
)
...
def _fleet_telemetrie_profile_status_laden():
    ...
    try:
        status["live_retry_attempts"] = max(
            0,
            int(geladen.get("live_retry_attempts") or 0),
        )
    except (TypeError, ValueError):
        pass
...
def _fleet_telemetrie_profile_live_takt_bestaetigt(data, status, jetzt=None):
    ...
    if not isinstance(vorher, dict):
        vorher = {}
    if not isinstance(abstände, dict):
        abstände = {}
    schnelle_felder = 0
    ...
    for feld in FLEET_TELEMETRIE_PROFILE_LIVE_BESTAETIGUNGSFELDER:
        ...
        if letzter is None:
            continue
        if vorletzter is None:
            intervall_ms = _as_float(abstände.get(feld))
            if intervall_ms is not None:
                vorletzter = letzter - (intervall_ms / 1000.0)
        if vorletzter is None:
            continue
        if vorletzter < letzter_versand or letzter < letzter_versand:
            continue
        if jetzt is not None:
            alter = float(jetzt) - letzter
            if (
                alter < 0
                or alter
...
def _fleet_telemetrie_profile_live_neuversand_fahrt_aktuell(status, jetzt):
    """Prüfe, ob eine laufende Live-Reparatur noch Fahrbewegung sieht."""
    ...
    if letzte_bewegung is None or letzte_bewegung <= 0:
        return False
    alter = float(jetzt) - letzte_bewegung
    return (
        0 <= alter
...
def _fleet_telemetrie_profile_live_wiederherstellungsprofil(
    ...
    if letztes_profil == "live":
        return "live_extended"
    if letztes_profil == "live_extended":
...
def _fleet_telemetrie_profile_live_takt_stabil(data, jetzt=None):
    ...
    for feld in FLEET_TELEMETRIE_PROFILE_LIVE_STABIL_FELDER:
        ...
        position_wiederaufgenommen = (
            feld in FLEET_TELEMETRIE_PROFILE_LIVE_FAHR_FELDER
            and 0 <= alter
            <= FLEET_TELEMETRIE_PROFILE_LIVE_POSITION_WIEDERAUFNAHME_SECONDS
        )
        if intervall_stabil or position_wiederaufgenommen:
            stabile_felder += 1
            if feld in FLEET_TELEMETRIE_PROFILE_LIVE_FAHR_FELDER:
...
def _fleet_telemetrie_profile_stream_bestaetigt(
    ...
    if profil in {"live", "live_extended"}:
        return _fleet_telemetrie_profile_live_takt_bestaetigt(
            data,
            status,
            jetzt,
        )
...
def _fleet_telemetrie_profile_status_an_daten(data):
    ...
    data["telemetry_live_retry_active"] = status.get("live_retry_active") is True
    data["telemetry_live_retry_moving"] = (
        _fleet_telemetrie_profile_live_neuversand_fahrt_aktuell(
            status,
            time.time(),
        )
    )
    data["telemetry_live_retry_started_at"] = status.get("live_retry_started_at")
    data["telemetry_live_retry_confirmed_at"] = status.get(
        "live_retry_confirmed_at"
    )
    data["telemetry_live_retry_attempts"] = int(
...
def _fleet_telemetrie_profile_sync_ausstehend_setzen(status, profil, jetzt):
    """Markiere ein gesendetes Profil bis zur Fahrzeugbestätigung als ausstehend."""

    status["last_sent"] = jetzt
    status["last_sent_profile"] = profil
    status["config_synced"] = False
    status["config_key_paired"] = None
    status["config_sync_state"] = "pending"
    status["config_sync_profile"] = profil
    status["config_sync_checked_at"] = jetzt
    status["config_sync_updated_at"] = jetzt
    status["config_sync_error"] = None
    status["config_sync_details"] = []
    status["updated_at"] = jetzt
...
def _fleet_telemetrie_profile_sync_erneut_pruefen():
    ...
    with _fleet_telemetry_profile_lock:
        ...
        if live_neuversand_aktiv:
            ...
            profil = (
                ...
                or status.get("config_sync_profile")
        ...
        if (
            neu_senden
            and profil in {"live", "live_extended"}
            and _fleet_telemetrie_profile_neuverbindung_erwartet(status, jetzt)
        ):
            neu_senden = False
        if (
            neu_senden
    ...
    with _fleet_telemetry_profile_lock:
        if (
            _fleet_telemetry_profile_status.get("config_sync_profile") != profil
            and _fleet_telemetry_profile_status.get("last_sent_profile") != profil
        ):
            return
...
def _fleet_telemetrie_profile_aktualisieren(cache_id, data):
    ...
    with _fleet_telemetry_profile_lock:
        ...
        live_takt_bestaetigt = (
            ziel == "live"
            and _fleet_telemetrie_profile_live_takt_bestaetigt(
                data,
                status,
                jetzt,
            )
        ...
        ziel_ausstehend = not ziel_bestaetigt
        neuversand_fällig = _fleet_telemetrie_profile_neuversand_fällig(
            status,
            jetzt,
            aktivierbares_ziel,
        )
        ...
        if (
            ...
            darf_senden = (
                ...
                and not _fleet_telemetrie_profile_neuverbindung_erwartet(
                    status,
                    jetzt,
                )
        ...
        if live_neuversand_pausiert:
            darf_senden = False
        if ziel_ausstehend and darf_senden:
    ...
    data["telemetry_live_retry_active"] = (
        status_kopie.get("live_retry_active") is True
    )
    data["telemetry_live_retry_moving"] = (
        _fleet_telemetrie_profile_live_neuversand_fahrt_aktuell(
            status_kopie,
    ...
    data["telemetry_live_retry_started_at"] = status_kopie.get(
        "live_retry_started_at"
    )
    data["telemetry_live_retry_confirmed_at"] = status_kopie.get(
        "live_retry_confirmed_at"
    )
    data["telemetry_live_retry_attempts"] = int(
        status_kopie.get("live_retry_attempts") or 0
    )
...
def _fleet_telemetrie_aktualitaetszeit_ms(data):
    """Liefere den jüngsten bestätigten Telemetrie- oder REST-Zeitpunkt."""
    ...
    for feld in (
        ...
        zeitpunkt = _as_float(data.get(feld))
        if isinstance(zeitpunkt, (int, float)):
            kandidaten.append(zeitpunkt)
    return max(kandidaten) if kandidaten else None


def _fleet_telemetrie_stream_ist_veraltet(data, jetzt_ms=None):
    """Erkenne einen vollständig ausgebliebenen Telemetrie-Datenstrom."""

    if not isinstance(data, dict):
        return False
    if jetzt_ms is None:
        jetzt_ms = time.time() * 1000
    stream_empfangen = _as_float(
        data.get("fleet_telemetry_received_at")
        or data.get("fleet_telemetry_updated_at")
    )
    if not isinstance(stream_empfangen, (int, float)):
        return False
    alter_seconds = max(0.0, (float(jetzt_ms) - stream_empfangen) / 1000.0)
    return (
        alter_seconds
...
def _fleet_telemetrie_streamprofil_wiederherstellung_anfordern(
    data,
    jetzt=None,
):
    """Starte bei einem stummen Stream die Live-Profilreparatur erneut."""

    jetzt = time.time() if jetzt is None else float(jetzt)
    if not _fleet_telemetrie_stream_ist_veraltet(data, jetzt * 1000):
        return False
    drive = data.get("drive_state") if isinstance(data, dict) else None
    if not isinstance(drive, dict):
        return False
    ...
    with _fleet_telemetry_profile_lock:
        ...
        if status.get("target") != "live":
            ...
            geändert = True
        if status.get("live_retry_motion_active") is not True:
            status["live_retry_motion_active"] = True
            geändert = True
...
def _fleet_telemetrie_stream_wiederherstellung_soll_aktualisiert_werden(
    data,
    jetzt_ms=None,
):
    """Erkenne aktive Fahrzeugdaten, die wegen eines Streamabbruchs festhängen."""

    if not _fleet_telemetrie_stream_ist_veraltet(data, jetzt_ms):
        return False
    if jetzt_ms is None:
        jetzt_ms = time.time() * 1000
    letzter_fallback = _as_float(
        data.get("fleet_vehicle_data_received_at")
    )
    if isinstance(letzter_fallback, (int, float)):
        alter = max(0.0, (float(jetzt_ms) - letzter_fallback) / 1000.0)
        if (
            alter
            < FLEET_TELEMETRIE_STREAM_WIEDERHERSTELLUNG_WIEDERHOLUNG_SECONDS
        ):
    ...
    if shift in {"R", "N", "D"}:
        return False
    if charge.get("charging_state") in {"Charging", "Starting"}:
        return True
    return any(
        _wert_ist_aktiv(climate.get(feld))
        for feld in (
            "is_climate_on",
            "is_auto_conditioning_on",
            "is_preconditioning",
...
def _fleet_telemetrie_stream_wiederherstellung_reservieren(vin, jetzt=None):
    """Drossele REST-Wiederherstellungen eines stummen Telemetrie-Streams."""
    ...
    with _fleet_telemetry_position_lock:
        letzte_abfrage = (
            _fleet_telemetry_stream_wiederherstellung_letzte_abfrage.get(
                str(vin)
            )
        )
        if (
            ...
            return False
        _fleet_telemetry_stream_wiederherstellung_letzte_abfrage[str(vin)] = jetzt
    return True
...
def _fleet_telemetrie_fahrzeugzustand_abrufen(vin):
    """Prüfe den Fahrzeugzustand über den nicht weckenden Fleet-Endpunkt."""
    ...
    antwort = payload.get("response") if isinstance(payload, dict) else None
    if not isinstance(antwort, dict):
        raise RuntimeError("Tesla lieferte keinen Fahrzeugzustand")
    return _normalisiere_dashboard_state(antwort.get("state"))


def _fleet_telemetrie_fallback_rohwerte_bereinigen(data, fahrzeugdaten):
    """Verwirf Rohwerte, die ein frischer REST-Datensatz ersetzt hat."""

    if not isinstance(data, dict) or not isinstance(fahrzeugdaten, dict):
        return
    rohwerte = data.get("fleet_telemetry_raw")
    if not isinstance(rohwerte, dict):
        return
    vehicle = fahrzeugdaten.get("vehicle_state")
    climate = fahrzeugdaten.get("climate_state")
    charge = fahrzeugdaten.get("charge_state")
    if not isinstance(vehicle, dict):
        vehicle = {}
    ...
    if not isinstance(charge, dict):
        charge = {}
    for raw_feld, owner_feld in FLEET_TELEMETRIE_FENSTER_FELDER.items():
        if owner_feld in vehicle:
            rohwerte.pop(raw_feld, None)
    if "is_rear_defroster_on" in climate:
        rohwerte.pop("RearDefrostEnabled", None)
    if "fan_status" in climate:
        rohwerte.pop("HvacFanSpeed", None)
        rohwerte.pop("HvacFanStatus", None)
    if "battery_heater_on" in charge or "battery_heater" in climate:
        rohwerte.pop("BatteryHeaterOn", None)


def _fleet_telemetrie_fallbackdaten_uebernehmen(
    vin,
    fahrzeugdaten,
    abgerufen_at_ms=None,
):
    """Übernehme einen vollständigen REST-Stand nach einem Streamabbruch."""
    ...
    with _fleet_telemetry_lock:
        for cache_id in _fleet_telemetrie_cache_ids(vin):
            ...
            for abschnitt in (
                ...
                quelle = fahrzeugdaten.get(abschnitt)
                if not isinstance(quelle, dict):
                    continue
            ...
            _fleet_telemetrie_fallback_rohwerte_bereinigen(data, fahrzeugdaten)
            ...
            for feld in (
                ...
                vehicle.pop(feld, None)
            shift = _normalize_shift_state(drive.get("shift_state"))
            if shift == "P":
                drive["shift_state"] = "P"
                if drive.get("speed") is None:
                    drive["speed"] = 0
                if drive.get("power") is None:
                    drive["power"] = 0
                data["fleet_telemetry_park_reconciled_at"] = int(
                    abgerufen_at_ms
                )
                _fleet_telemetrie_navigation_beenden(
                    drive,
            ...
            state = _normalisiere_dashboard_state(fahrzeugdaten.get("state"))
            if state is not None:
                data["state"] = state
            if vorheriger_state != data.get("state"):
                data["state_since_ms"] = int(abgerufen_at_ms)
                data["state_since_at"] = int(abgerufen_at_ms)
            data["state_checked_at"] = int(abgerufen_at_ms)
            data["fleet_vehicle_data_received_at"] = int(abgerufen_at_ms)
            data["fleet_vehicle_data_source"] = "stream_recovery"
            zeitpunkte = [int(abgerufen_at_ms)]
            for abschnitt in (
                "drive_state",
                "vehicle_state",
                "climate_state",
                ...
                if not isinstance(quelle, dict):
                    continue
                zeitpunkt = _fleet_telemetrie_zeitstempel_ms(
                    quelle.get("timestamp")
                )
                if isinstance(zeitpunkt, (int, float)):
                    zeitpunkte.append(int(zeitpunkt))
            data["timestamp"] = max(zeitpunkte)
            data["_live"] = data.get("state") == "online"
...
def _fleet_telemetrie_position_worker_loop():
    ...
    while _fleet_telemetrie_aktiv():
        ...
        for vehicle in _fleet_telemetrie_fahrzeuge():
            ...
            _fleet_telemetrie_streamprofil_wiederherstellung_anfordern(data)
            if _fleet_telemetrie_stream_wiederherstellung_soll_aktualisiert_werden(
                data
            ):
...
def _fleet_telemetrie_cache_fuer_dashboard(cache_id, cached=None):
    ...
    if not isinstance(data, dict):
        return None
    updated_at = _fleet_telemetrie_aktualitaetszeit_ms(data)
...
def _normalize_charge_soc(value):
    """Normalisiere einen SOC-Wert als Ganzzahl (Prozent)."""
    if value is None:
        return None
    try:
...
def _extract_current_charge_soc(charge_state):
    """Liefert den aktuellen SOC aus ``charge_state``."""
    if not isinstance(charge_state, dict):
        return None
    pct = charge_state.get("usable_battery_level")
    if pct is None:
...
def _load_session_start_soc(vehicle_id):
    ...
    if start_soc is not None:
        return start_soc
    try:
        with open(_session_start_soc_file(vehicle_id), "r", encoding="utf-8") as f:
            value = f.read().strip()
        if value == "":
            return None
        start_soc = _normalize_charge_soc(value)
        if start_soc is None:
            return None
        _charging_session_start_soc[vehicle_id] = start_soc
        return start_soc
    except Exception:
        return None
...
def _save_session_start_soc(vehicle_id, start_soc):
    """Speichere den Start-SOC der Ladesession."""
    start_soc = _normalize_charge_soc(start_soc)
    if start_soc is None:
        return
    try:
        os.makedirs(vehicle_dir(vehicle_id), exist_ok=True)
        with open(_session_start_soc_file(vehicle_id), "w", encoding="utf-8") as f:
            f.write(str(start_soc))
        _charging_session_start_soc[vehicle_id] = start_soc
    except Exception:
        pass
...
def _clear_session_start_soc(vehicle_id):
    """Lösche den Start-SOC der Ladesession."""
...
def _apply_charge_session_payload(charge_state, session_start, session_start_soc, now):
    """Ergänze ``charge_state`` mit Ladesession-Informationen."""
    if not isinstance(charge_state, dict):
        return
    if session_start is not None:
        charge_state["charge_session_start"] = session_start.isoformat()
        try:
            duration = int(max(0, (now - session_start).total_seconds()))
        except Exception:
            duration = None
    else:
        charge_state.pop("charge_session_start", None)
        duration = None

    if duration is not None:
        charge_state["charge_session_duration_s"] = duration
    else:
        charge_state.pop("charge_session_duration_s", None)

    if session_start_soc is not None:
        charge_state["charge_session_start_soc"] = session_start_soc
    else:
        charge_state.pop("charge_session_start_soc", None)

    current_soc = _extract_current_charge_soc(charge_state)
    if current_soc is not None and session_start_soc is not None:
...
def get_vehicle_state(vehicle_id=None):
    ...
    if _fleet_telemetrie_aktiv():
        ...
        if isinstance(telemetry_data, dict):
            ...
            checked_at = _fleet_telemetrie_aktualitaetszeit_ms(telemetry_data)
...
def _fetch_data_once(vehicle_id="default"):
    ...
    if isinstance(data, dict):
        ...
        if session_ended:
            ...
            if end_soc_letzter is not None:
                _save_last_charge_end_soc(cache_id, end_soc_letzter)
                saved_end_soc = end_soc_letzter
...
def api_data():
    ...
    if data is None:
        data = _fetch_data_once("default")
    telemetry_data = _fleet_telemetrie_cache_fuer_dashboard("default", data)
    if telemetry_data is not None:
        data = telemetry_data
    _fleet_telemetrie_rohdaten_anreichern(data)
...
def api_data_vehicle(vehicle_id):
    ...
    if data is None:
        data = _fetch_data_once(vehicle_id)
    telemetry_data = _fleet_telemetrie_cache_fuer_dashboard(vehicle_id, data)
    if telemetry_data is not None:
        data = telemetry_data
    _fleet_telemetrie_rohdaten_anreichern(data)
    _fahrtpfad_nach_parkzeit_bereinigen(data)
        ...
        try:
            ...
            if vehicle_id in latest_data:
                ...
                if initial_data is None:
                    initial_data = latest_data[vehicle_id]
                initial = _subscriber_stream_payload(initial_data)
